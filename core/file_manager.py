...
GOLD_DATA_DIR = os.path.join("data", "gold")
...
ESTRUTURA_DADOS = {
    "Mercado Livre": { 
        "Vendas (Geral)": {"skiprows": 5}, 
        "Repasse Financeiro (Mercado Pago)": {"skiprows": 0},
        "Clientes ERP": {"skiprows": 0}  # NOVA ESTRUTURA RECONHECIDA
    },
...
def auto_detectar_tipo_e_pulo(marketplace, filename):
    ...
    # DETECÇÃO INTELIGENTE DO ERP
    if "cliente" in nome_min or "crm" in nome_min:
        return "Clientes ERP", 0
        
    if marketplace == "Mercado Livre":
        if "collection" in nome_min or "repasse" in nome_min: 
            return "Repasse Financeiro (Mercado Pago)", 0
        else: 
            return "Vendas (Geral)", 5 
...
def ler_amostra(uploaded_file, marketplace, sheet_name, nrows=10) -> pd.DataFrame:
    ...
    if uploaded_file.name.endswith('.csv'): 
        df = pd.read_csv(uploaded_file, skiprows=pular, nrows=nrows, sep=';', encoding='latin1', on_bad_lines='skip', dtype=str)
        # Remove o artefacto BOM das colunas na amostragem
        df.columns = df.columns.str.replace('ï»¿', '', regex=False)
    else: 
        df = pd.read_excel(uploaded_file, sheet_name=sheet_name, skiprows=pular, nrows=nrows, engine='calamine')
...
def processar_carga_incremental(uploaded_file, marketplace, sheet_name, usuario, privilegio) -> tuple[int, int]:
    ...
    if uploaded_file.name.endswith('.csv'): 
        df_novo = pd.read_csv(caminho_raw, skiprows=pular, sep=';', encoding='latin1', on_bad_lines='skip', dtype=str)
        df_novo.columns = df_novo.columns.str.replace('ï»¿', '', regex=False)
    else: 
        df_novo = pd.read_excel(caminho_raw, sheet_name=sheet_name, skiprows=pular, engine='calamine')
    
    df_novo.dropna(how='all', axis=1, inplace=True)
    df_novo.dropna(how='all', axis=0, inplace=True)
    
    # CORREÇÃO ABSOLUTA: Força tudo para String na base nova
    ...
    # SEGUNDA BLINDAGEM: Garante que a tabela consolidada final esteja 100% como texto 
    # antes de serializar para o Parquet (evita erros do PyArrow)
...
def executar_pipeline_ouro(marketplace, df_bruto):
    ...
    """
    # BLINDAGEM AUTOMÁTICA: Corrige colunas de dados do histórico que já continham o erro
    df_bruto.columns = df_bruto.columns.str.replace('ï»¿', '', regex=False)

    df_vendas_bruto = df_bruto[df_bruto['Tipo_Relatorio'] == 'Vendas (Geral)'].copy()
    df_mp_bruto = df_bruto[df_bruto['Tipo_Relatorio'] == 'Repasse Financeiro (Marketplace Pago)' if 'Repasse Financeiro (Marketplace Pago)' in df_bruto['Tipo_Relatorio'].values else df_bruto['Tipo_Relatorio'] == 'Repasse Financeiro (Mercado Pago)'].copy()
    
    # Extração inteligente do ERP
    df_erp_bruto = df_bruto[df_bruto['Tipo_Relatorio'] == 'Clientes ERP'].copy()
    ...
    # 2. Busca Inteligente MP
    df_prata_unificada = etl_engine.unificar_vendas_repasse(df_vendas_prata, df_mp_prata)
    
    # 3. Extração (Passando o ERP para a Função MDM)
    d_clientes = etl_engine.extrair_dclientes(df_prata_unificada, df_erp=df_erp_bruto)
    d_produtos = etl_engine.extrair_dprodutos(df_prata_unificada)
    d_logistica = etl_engine.extrair_dlogistica(df_prata_unificada)
    f_vendas = etl_engine.extrair_fvendas(df_prata_unificada, df_erp=df_erp_bruto)
    d_calendario = etl_engine.extrair_dcalendario(f_vendas)
    ...
    if not d_calendario.empty:
        d_calendario.to_parquet(os.path.join(GOLD_DATA_DIR, f"dim_calendario_{mk_limpo}.parquet"), index=False)
    ...
    resumo = {
        ...
        "Dias (dCalendario)": len(d_calendario)
    }
